...
import {
  Server, Key, RefreshCw, Send, Play, Terminal, Database, ShieldAlert, CheckCircle,
  XCircle, AlertTriangle, HelpCircle, Copy, Info, Check, ArrowRight, Truck, Wallet,
} from "lucide-react";
import { runDiagnostics } from "../services/api-diagnostics";
import { dispatchWebhook } from "../services/webhook-dispatcher";
import { simulateMercadoPagoPayment, simulateMelhorEnvioFreight, startMelhorEnvioOAuth } from "../services/integrations-simulator";
...
export function DevConsoleDashboard() {
  ...
  const { data: diagnostics, isFetching: isCheckingHealth, refetch: refetchHealth } = useQuery<ApiHealthStatus[]>({
  ...
  const { data: orders } = useQuery<any[]>({
    ...
    queryFn: async () => {
      try {
        ...
        return [
          { id: "ped_mock_01", total: 189.90, status: "pendente", created_at: new Date().toISOString(), customer_id: { name: "Maria Oliveira (Mock)" } },
          { id: "ped_mock_02", total: 420.00, status: "pendente", created_at: new Date().toISOString(), customer_id: { name: "Carlos Souza (Mock)" } },
          { id: "ped_mock_03", total: 95.00, status: "pago", created_at: new Date().toISOString(), customer_id: { name: "Ana Silva (Mock)" } },
        ];
      }
    }
  });
    ...
    async function loadSettings() {
      try {
        ...
        if (saved) {
          ...
          if (parsed.n8n) { setN8nUrl(parsed.n8n.webhook_url); setN8nMode(parsed.n8n.mode); }
          if (parsed.me) { setMeClientId(parsed.me.clientId); setMeMode(parsed.me.mode); }
          if (parsed.mp) { setMpPublicKey(parsed.mp.public_key); setMpPrivateKey(parsed.mp.private_key); setMpMode(parsed.mp.mode); }
        }
      }
    }
    loadSettings();
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async ({ provider, settings }: { provider: string; settings: Partial<IntegrationSettings> }) => {
      if (fallbackMode) {
        // Salva localmente
        const saved = localStorage.getItem("dev_integration_settings") || "{}";
        const parsed = JSON.parse(saved);
      ...
      const payload: any = {
        ...
        api_url: provider === "supabase" 
          ? import.meta.env.VITE_SUPABASE_URL 
          : provider === "melhor_envio" 
            ? (settings.mode === "production" ? "https://api.melhorenvio.com.br" : "https://sandbox.melhorenvio.com.br")
            : null,
    ...
    <AdminShell title="Ambiente Dev (Developer Console)">
      {fallbackMode && (
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-amber-600 dark:text-amber-400">
          ...
          <div className="text-sm">
            <span className="font-semibold">Modo Fallback Ativo:</span> As tabelas de configuração do banco (`integration_settings` ou `webhook_logs`) não foram detectadas. A aplicação está simulando o painel de dev perfeitamente utilizando <span className="font-mono">localStorage</span> e estados locais de memória!
          </div>
        </div>
      )}
      ...
      <div className="mb-6 grid gap-4 grid-cols-2 md:grid-cols-4">
        {diagnostics?.map((health) => {
          ...
          return (
            <Card key={health.provider} className="overflow-hidden border border-border bg-card p-4 transition-colors">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  {health.provider === "mercado_pago" ? "Mercado Pago" : health.provider.toUpperCase()}
                </span>
      ...
      <Tabs defaultValue="diagnostics" className="w-full">
        <TabsList className="mb-6 grid w-full grid-cols-4 rounded-xl bg-muted p-1">
          <TabsTrigger value="diagnostics" className="rounded-lg flex items-center gap-1.5"><Key className="h-4 w-4" /> APIs & Chaves</TabsTrigger>
          <TabsTrigger value="simulators" className="rounded-lg flex items-center gap-1.5"><Send className="h-4 w-4" /> Simuladores</TabsTrigger>
          <TabsTrigger value="webhooks" className="rounded-lg flex items-center gap-1.5"><Terminal className="h-4 w-4" /> N8N Webhooks</TabsTrigger>
          <TabsTrigger value="database" className="rounded-lg flex items-center gap-1.5"><Database className="h-4 w-4" /> Supabase</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="diagnostics">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="border-border bg-card">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Configure a rota de webhook para disparar fluxos no N8N</CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardFooter>
                <Button 
                  onClick={() => saveSettingsMutation.mutate({ provider: "n8n", settings: { webhook_url: n8nUrl, mode: n8nMode } })}
            ...
            <Card className="border-border bg-card">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Sandbox ou credenciais de produção para pagamentos Pix</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
              ...
              <CardFooter>
                <Button 
                  onClick={() => saveSettingsMutation.mutate({ provider: "mercado_pago", settings: { public_key: mpPublicKey, private_key: mpPrivateKey, mode: mpMode } })}
                  className="rounded-xl w-full"
                >
            ...
            <Card className="border border-border bg-card md:col-span-2">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Fluxo visual de autorização exigido pelo Melhor Envio para gerar tokens de cotação e emissão de fretes</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
              ...
              <CardFooter className="flex flex-col sm:flex-row gap-3">
                <Button 
                  variant="outline"
                  onClick={() => saveSettingsMutation.mutate({ provider: "melhor_envio", settings: { public_key: meClientId, mode: meMode } })}
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Ligue pedidos pendentes mockando a liquidação do gateway</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  >
                    ...
                    {orders?.map((o) => (
                      <option key={o.id} value={o.id}>
                        {o.id.substring(0, 12)}... - {o.customer_id?.name || "Sem Nome"} (R$ {o.total} - {o.status.toUpperCase()})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">Requer credenciais reais salvas</span>
                  </div>
                  <Switch 
                    checked={simMpRealSandbox} 
                    onCheckedChange={setSimMpRealSandbox}
                  />
                </div>
              </CardContent>
              <CardFooter>
                <Button 
                  ...
                  className="rounded-xl w-full"
                >
                  {paymentMutation.isPending ? "Processando Simulação..." : "Confirmar & Processar Transação de Teste"}
                </Button>
              </CardFooter>
            </Card>
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Simule cotações para CEPs com mocks ou via API de homologação</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex flex-col">
                    <span className="text-sm font-medium">Bater na API Sandbox Real</span>
                    <span className="text-xs text-muted-foreground">Exige chaves válidas salvas</span>
                  </div>
                  <Switch 
                    checked={simMeRealSandbox} 
                    onCheckedChange={setSimMeRealSandbox}
                  />
                </div>
                ...
                {freightOptions.length > 0 && (
                  <div className="mt-4 border-t border-border pt-4 space-y-2">
                    <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Opções de Envio Disponíveis</span>
                    <div className="grid gap-2 max-h-[180px] overflow-y-auto pr-1">
                      {freightOptions.map((opt) => (
                        <div key={opt.id} className="flex items-center justify-between rounded-xl border border-border p-3 hover:bg-muted/40 transition-colors">
                          <div className="flex items-center gap-2">
                            {opt.company.picture && (
                              <img src={opt.company.picture} alt={opt.company.name} className="h-6 w-10 object-contain rounded bg-white p-0.5" />
                            )}
                            <div>
                              <div className="text-sm font-semibold">{opt.name}</div>
                              <div className="text-xs text-muted-foreground">Prazo: {opt.delivery_time} {opt.delivery_time === 1 ? "dia útil" : "dias úteis"}</div>
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-sm font-semibold text-primary">R$ {opt.custom_price.toFixed(2)}</div>
                            <div className="text-[10px] text-muted-foreground line-through">R$ {opt.price.toFixed(2)}</div>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Escolha o evento e dispare um payload sob demanda para testar o N8N</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (ev === "vendas.pedido_criado" || ev === "vendas.pedido_pago") {
                        setManualPayload(JSON.stringify({ order_id: "order_9988_mock", total: 350.00, status: ev.endsWith("pago") ? "pago" : "pendente", customer: { name: "Cliente Exemplo", email: "cliente@exemplo.com" } }, null, 2));
                      } else if (ev === "crm.lead_adicionado") {
                        setManualPayload(JSON.stringify({ lead_id: "lead_uuid_123", name: "Amanda Pereira B2B", email: "amanda@empresa.com", origin: "formulario_contato" }, null, 2));
                      } else if (ev === "fidelidade.resgate_efetuado") {
                        setManualPayload(JSON.stringify({ customer_id: "cust_992", points_redeemed: 500, reward_name: "Cupom R$ 50", code: "COMAMOR50" }, null, 2));
                      }
                    }}
            ...
            <Card className="border border-border bg-card lg:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>Auditoria em tempo real de webhooks despachados e integrados com N8N</CardDescription>
                </div>
                <Button variant="ghost" size="icon" onClick={() => refetchLogs()} className="rounded-xl">
                    ...
                    {webhookLogs && webhookLogs.length > 0 ? (
                      webhookLogs.map((log) => {
                        ...
                        return (
                          ...
                          >
                            <TableCell>
                              <Badge className={`rounded-lg px-2 py-0.5 font-semibold ${
                                isSuccess 
                            ...
                            <TableCell className="font-mono text-xs font-semibold">{log.event_type}</TableCell>
              ...
              >
                <div className="space-y-6 overflow-y-auto pr-1 flex-1">
                  <div className="flex items-center justify-between border-b border-border pb-4">
                    <div>
                      <h3 className="font-display text-lg font-semibold">Detalhes do Evento Webhook</h3>
                      <p className="text-xs text-muted-foreground font-mono mt-0.5">{selectedLog.id}</p>
                    </div>
                    <Button variant="ghost" onClick={() => setSelectedLog(null)} className="rounded-xl">Fechar</Button>
                  </div>

                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="text-xs text-muted-foreground">Evento</span>
                      <div className="font-mono font-semibold text-primary">{selectedLog.event_type}</div>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">Status do Gateway</span>
                      <div>
                        <Badge className={`rounded-lg px-2 py-0.5 ${
                          selectedLog.status === "success" 
                            ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20" 
                            : "bg-rose-500/10 text-rose-500 border border-rose-500/20"
                        }`}>
                          {selectedLog.status_code === 0 ? "CORS / NETWORK ERR" : `${selectedLog.status_code} - ${selectedLog.status.toUpperCase()}`}
                        </Badge>
                      </div>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">URL Destino (N8N)</span>
                      <div className="truncate font-mono text-xs text-muted-foreground" title={selectedLog.webhook_url}>{selectedLog.webhook_url}</div>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">Tempo de Resposta (N8N)</span>
                      <div className="font-mono text-xs text-muted-foreground">{selectedLog.duration_ms ? `${selectedLog.duration_ms}ms` : "Sem resposta"}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                        <Terminal className="h-3.5 w-3.5 text-primary" /> Request Body (Payload Enviado)
                      </span>
                      <Button 
                        size="sm" 
                        variant="ghost" 
                        onClick={() => copyToClipboard(JSON.stringify(selectedLog.payload, null, 2), "req-copy")}
                        className="h-7 text-xs rounded-lg"
                      >
                        {copiedId === "req-copy" ? <Check className="h-3 w-3 text-emerald-500 mr-1" /> : <Copy className="h-3 w-3 mr-1" />}
                        Copiar JSON
                      </Button>
                    </div>
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Acompanhe o histórico de schemas e migrações executadas na plataforma</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-xl border border-border p-4 bg-muted/20 space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <Badge className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-lg">ATIVO (POSTGRESQL)</Badge>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-mono text-xs text-primary font-semibold">20260518000000_ambiente_dev_core</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Migrações do Projeto (supabase/migrations/*)</span>
                  <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
                    {[
                      ...
                      { ver: "20260512004617", desc: "c8cfb13f_db_initial_setup.sql", active: true },
                    ].map((mig) => (
                      <div key={mig.ver} className="flex items-center justify-between text-xs rounded-lg border border-border p-2 bg-secondary/20">
                        <div className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-emerald-500" />
                          <span className="font-mono text-muted-foreground">{mig.ver}</span>
                          <span className="font-medium text-foreground truncate max-w-[150px]">{mig.desc}</span>
                        </div>
                        <Badge variant="outline" className="rounded-lg bg-emerald-500/5 text-emerald-500 border-emerald-500/10 text-[9px]">Aplicado</Badge>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Verifique se os microsserviços Deno em nuvem estão responsivos</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  {[
                    { name: "dispatch-webhook", route: "/functions/v1/dispatch-webhook", desc: "Garante entrega em lote assíncrona", status: "online", latency: 85 },
                    { name: "sync-stock-erp", route: "/functions/v1/sync-stock-erp", desc: "Cron job para sincronizar estoque físico", status: "online", latency: 120 },
                    { name: "calculate-freight", route: "/functions/v1/calculate-freight", desc: "Servidor proxy Melhor Envio", status: "warning", latency: 450, msg: "Token expirando em 2 dias" },
                  ].map((func) => {
                    const isOnline = func.status === "online";
                    return (
                      <div key={func.name} className="rounded-xl border border-border p-3 hover:bg-muted/30 transition-colors flex items-center justify-between">
                        <div className="space-y-0.5">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold font-mono">{func.name}</span>
                            <Badge className={`rounded-lg text-[9px] px-1.5 py-0.2 font-semibold ${
                          ...
                          <div className="text-[9px] text-muted-foreground font-mono truncate max-w-[200px]">{func.route}</div>
                        </div>
                        <div className="text-right">
