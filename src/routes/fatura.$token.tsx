    ...
    <div className="min-h-screen bg-background pb-12">
      <header className="border-b border-border">
        <div className="mx-auto flex h-14 max-w-2xl items-center gap-2 px-4 sm:h-16 sm:px-6">
          ...
          <span className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">fatura</span>
        </div>
      </header>
        ...
        <div className="rounded-3xl border border-border bg-card p-5 sm:p-8">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h1 className="mt-1 font-display text-2xl sm:text-3xl">Olá, {invoice.customers?.name ?? "cliente"}!</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {invoice.orders?.code && <>Pedido <strong>{invoice.orders.code}</strong> · </>}
          ...
          <div className="mt-6 rounded-2xl bg-accent/30 p-5 text-center">
            ...
            <p className="mt-1 font-display text-4xl text-primary sm:text-5xl">{brl(Number(invoice.total))}</p>
          </div>
            ...
            <div className="mt-6 space-y-4">
              {invoice.payment_method === "pix" && invoice.pix_copia_cola && (
                <div className="rounded-2xl border border-border p-5">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Copie o código abaixo e cole no app do seu banco.</p>
                  <div className="mt-3 break-all rounded-xl bg-muted p-3 font-mono text-[11px]">
                    {invoice.pix_copia_cola}
                  </div>
                  <button onClick={copyPix} className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground">
              ...
              {invoice.payment_method === "boleto" && invoice.boleto_url && (
                <div className="rounded-2xl border border-border p-5">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Vencimento {invoice.due_date ? dateBR(invoice.due_date) : "—"}</p>
                  <a
              ...
              {invoice.payment_method === "cartao" && invoice.mp_init_point && (
                <div className="rounded-2xl border border-border p-5">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Pagamento seguro via Mercado Pago.</p>
                  <a
