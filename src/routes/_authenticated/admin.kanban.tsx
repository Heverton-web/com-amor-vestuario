...
function KanbanPage() {
  ...
  const { data: cards } = useQuery({
    queryKey: ["kanban", tab],
    queryFn: async () => (await supabase.from("kanban_cards").select("*").eq("board", tab).order("position")).data ?? [],
  });
  ...
  const move = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["kanban", tab] }); toast.success("Card movido"); },
  });
    ...
    <AdminShell title="CRM · Kanbans">
                ...
                <div className="space-y-2">
                  {stageCards.map((c: any) => (
                    ...
                    >
                      ...
                      {c.contact_whatsapp && <div className="mt-1 text-xs text-muted-foreground">📱 {c.contact_whatsapp}</div>}
                      {c.amount && <div className="mt-1 text-xs font-medium text-primary">{brl(Number(c.amount))}</div>}
                      ...
                      >
                        {board.stages.map((st) => <option key={st.key} value={st.key}>→ {st.label}</option>)}
                      </select>
                    </button>
                  ))}
                  {!stageCards.length && (
      ...
      {detail && (
        <CardDetailDialog
          ...
          onMove={(stage) => { move.mutate({ id: detail.id, stage }); setDetail({ ...detail, stage }); }}
        />
      )}
    </AdminShell>
  );
...
export function CardDetailDialog({ card, stages, onClose, onMove }: { card: any; stages: { key: string; label: string }[]; onClose: () => void; onMove: (stage: string) => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/60 p-0 backdrop-blur-sm md:items-center md:p-6" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-t-3xl border border-border bg-background p-5 md:rounded-3xl md:p-7">
        ...
        <div className="mt-4 space-y-3 text-sm">
          ...
          {card.contact_whatsapp && (
            <div className="flex items-center justify-between">
              ...
              <a href={`https://wa.me/${card.contact_whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer"
        ...
        <div className="mt-5">
          <label className="mb-1 block text-xs uppercase tracking-wider text-muted-foreground">Etapa</label>
          <select value={card.stage} onChange={(e) => onMove(e.target.value)}
