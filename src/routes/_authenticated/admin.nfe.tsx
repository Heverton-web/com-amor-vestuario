...
function NfePage() {
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .in("status", ["pago", "enviado", "finalizado"])
      ).data ?? [],
  });
  ...
  function whatsApp(o: any) {
    ...
    const text = encodeURIComponent(
      `Olá ${o.customers?.name ?? ""}! Segue a NF-e do pedido ${o.code} no valor de ${brl(Number(o.total))}. — Com Amor Vestuário`,
    );
  ...
  return (
    <AdminShell title="Nota Fiscal Eletrônica">
      <div className="mb-6 rounded-2xl border border-border bg-accent/30 p-4 text-sm text-muted-foreground">
        ℹ️ Gere e envie NF-e dos pedidos faturados/finalizados. <strong>Demonstração</strong> —
        integração real com NFePHP, Focus NFe ou eNotas pode ser plugada substituindo a função{" "}
        <code>generate()</code>.
      </div>
              ...
              <tr key={o.id} className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-1">
                    ...
                    >
                      <Receipt className="h-3 w-3" /> Gerar
                    </button>
                    <button
                      onClick={() => generate(o)}
