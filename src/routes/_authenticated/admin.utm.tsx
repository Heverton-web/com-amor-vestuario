    ...
    <AdminShell title="Gerador de UTM">
      <div className="grid gap-6 md:grid-cols-[1fr_1fr]">
        <div className="space-y-4 rounded-2xl border border-border bg-card p-6">
          ...
          <Lbl t="utm_campaign * (campanha)">
            <input
              value={campaign}
              onChange={(e) => setCampaign(e.target.value)}
              placeholder="lancamento_inverno_2026"
              className="input"
            />
          </Lbl>
          <Lbl t="utm_content (variação do criativo)">
            <input value={content} onChange={(e) => setContent(e.target.value)} className="input" />
          </Lbl>
          <Lbl t="utm_term (palavra-chave)">
        ...
        <div className="rounded-2xl border border-border bg-primary p-6 text-primary-foreground">
          ...
          <p className="mt-4 break-all rounded-xl bg-primary-foreground/10 p-4 font-mono text-sm">
            {final}
          </p>
          <button
            onClick={copy}
            className="mt-4 inline-flex items-center gap-2 rounded-full bg-primary-foreground px-5 py-2.5 text-sm font-medium text-primary"
          >
            <Copy className="h-4 w-4" /> Copiar link
          </button>
          <div className="mt-6 space-y-2 text-sm opacity-80">
            <p>
              <Check className="mr-1 inline h-3 w-3" /> Slugs normalizados (lowercase, sem acento)
            </p>
            <p>
              <Check className="mr-1 inline h-3 w-3" /> Padrão GA4 / Meta / Google Ads
            </p>
            <p>
              <Check className="mr-1 inline h-3 w-3" /> Tracking automático de leads no formulário
            </p>
          </div>
        </div>
      </div>
