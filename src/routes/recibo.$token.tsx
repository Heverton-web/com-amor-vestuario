...
const METHOD_LABEL: Record<string, string> = {
  ...
  transferencia: "Transferência bancária", boleto: "Boleto", outro: "Outro",
};
...
function PublicReceiptPage() {
  ...
  const rc = r as {
    code: string; amount: number; amount_in_words: string; payer_name: string;
    payer_doc: string | null; reference: string; payment_method: string;
    paid_at: string; city: string | null; issuer_name: string | null;
    issuer_doc: string | null; issuer_address: string | null;
  ...
  const handlePDF = () =>
    downloadReceiptPDF({
      ...
      publicUrl: typeof window !== "undefined" ? `${window.location.origin}/recibo/${rc.public_token}` : "",
    });
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="flex items-center justify-between border-b border-border bg-muted/40 px-6 py-4">
            ...
            <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium ${cancelled ? "bg-muted text-muted-foreground" : "bg-primary text-primary-foreground"}`}>
          ...
          <div className="space-y-5 p-6">
            <div className="text-center">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Valor recebido</div>
            ...
            <div className="space-y-2 rounded-xl bg-muted/30 p-4 text-sm">
              <Row k="Recebi de" v={`${rc.payer_name}${rc.payer_doc ? ` (${rc.payer_doc})` : ""}`} />
              <Row k="Referente a" v={rc.reference || "—"} />
              <Row k="Forma de pagamento" v={METHOD_LABEL[rc.payment_method] ?? rc.payment_method} />
          ...
          <div className="border-t border-border bg-background px-6 py-4">
            <button onClick={handlePDF} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground">
