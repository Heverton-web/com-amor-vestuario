...
- **Fórmula de Conversão:**
  ...
  - _Exemplo:_ Se a taxa configurada for de **R$ 10,00 = 1 ponto** (parâmetro `points_per_real` nas configurações do site):
...
O cliente gerencia seu relacionamento com a marca através de uma interface exclusiva, otimizada para dispositivos móveis e com estética _Premium_ (Glassmorphism e tons suaves).
...
3. **Cálculo do Desconto:**
   - Se for _Frete Grátis_, zera o valor do frete.
   - Se for _Valor Fixo_, desconta o valor do cupom (limitado ao subtotal).
   - Se for _Percentual_, calcula a porcentagem correspondente sobre o valor das peças.
...
- **Gestão Manual:** Se necessário (como em casos de devolução física ou suporte ao cliente), o administrador pode alterar o status do resgate de forma manual para: _Resgatado, Utilizado, Expirado ou Cancelado_.
